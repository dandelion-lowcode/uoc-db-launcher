 ...
 *
 * <p>Unlike the other clients, redis-cli prints everything the same way, so a refused
 * command reads exactly like a result and is easy to scroll straight past. The mark added
 * here is the same one the other consoles already show, and it is put only where
...
final class RedisOutputHighlighter {
     ...
     * @return the same text with refusals marked; every visible character is unchanged
     */
    static String highlight(String output) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            result.append(ERROR_LINE.matcher(lines[i]).matches()
                    ? ERROR + lines[i] + RESET
                    : lines[i]);
        }
